...
public class CollegeServiceImpl implements CollegeService {
	...
	long minId;
	@Value("${app.person.id.min:999999}")
	long maxId;
	...
	public PersonDto addStudent(PersonDto person) {
		...
		if(studentRepo.existsById(student.getId())) {
			throw new IllegalStateException("Lecturer with given id already exsists" + student.getId());
		}
	...
	public PersonDto addLecturer(PersonDto person) {
		...
		if(lecturerRepo.existsById(lecturer.getId())) {
			throw new IllegalStateException("Lecturer with given id already exsists" + lecturer.getId());
		}
