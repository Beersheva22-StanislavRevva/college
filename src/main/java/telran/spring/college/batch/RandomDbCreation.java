...
public class RandomDbCreation {
	...
	void createDb() {
		if (creationDb.equals("create")) {
			...
			if (countErrors > 0) {
				...
				log.debug("DB created with no errors");
			}
		} else {
	...
	private void createMarks() {
		...
		Stream.generate(() -> getRandomMarkDto()).limit(nMarks).forEach(mark -> {
			try {
	...
	private MarkDto getRandomMarkDto() {
	...
	private String randomBirthDate() {
		int year = getRandomNumber(1990, 2001);
